...
func (e *Exporter) getClusterNodes(c redis.Conn) ([]string, error) {
	...
	for _, line := range lines {
		if node, ok := parseClusterNodeString(line, e.options.ClusterDiscoveryPreferHostnames); ok {
...
*/
func parseClusterNodeString(node string, preferHostname bool) (string, bool) {
	...
	if preferHostname && len(address) >= 5 && address[4] != "" {
