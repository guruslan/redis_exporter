...
func main() {
	var (
		...
		clusterDiscoveryPreferHostnames = flag.Bool("cluster-discovery-prefer-hostnames", getEnvBool("REDIS_EXPORTER_CLUSTER_DISCOVERY_PREFER_HOSTNAMES", false), "Prefer hostnames from the CLUSTER NODES output for the discovery endpoint when available.")
	...
	exp, err := exporter.NewRedisExporter(
		*redisAddr,
		exporter.Options{
			...
			IsTile38:                        *isTile38,
			IsCluster:                       *isCluster,
			ClusterDiscoveryPreferHostnames: *clusterDiscoveryPreferHostnames,
